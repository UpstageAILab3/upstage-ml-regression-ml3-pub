...
project_name = wandb.run.project + "-trail-7th"
...
def train_model(model, dataloader, criterion, optimizer, num_epochs=25, log_interval=10, checkpoint_interval=100):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        epoch_rmse = np.sqrt(np.mean(epoch_losses))
...
plt.hist(train_data['target'], bins=50, edgecolor='k')
...
# '계약년월일' 컬럼 삭제
train_data = train_data.drop(columns=['계약년월일'])
test_data = test_data.drop(columns=['계약년월일'])
