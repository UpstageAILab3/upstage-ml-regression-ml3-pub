...
hidden_units = [128, 64]  # Increased number of units
dropout_type = 'normal'
hidden_activation = 'relu'  # Changed activation function to ReLU
early_stopping = 5
...
def preprocess_data(df, include_target=False):
    ...
    categorical_columns = ['시군구', '번지', '아파트명', '시', '구', '동']
    df = one_hot_encode(df, columns=categorical_columns)
    
    # Apply Ridit transformation to continuous variables
    ridit_columns = ['전용면적(㎡)', '계약년월', '건축년도']
    df = smooth_ridit_transform(df, columns=ridit_columns)
...
def train_model(model, dataloader, criterion, optimizer, num_epochs=25, log_interval=10, checkpoint_interval=1):
    model.train()
    rmse_history = []
    batch_rmse_history = []
    best_rmse = float('inf')
